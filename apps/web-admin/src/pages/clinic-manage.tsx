  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4">
      ...
      <div className="flex flex-wrap gap-1 border-b pb-2">
        {tabs.map((t) => (
          ...
          >
            ...
            {t.label}
            {t.ent && !isEnterprise && <Lock className="h-3 w-3 opacity-60" />}
          </button>
        ))}
      </div>
      ...
      <div className="bg-card rounded-xl border p-4">
        ...
        {tab === 'branches' &&
          (isEnterprise ? <BranchesTab clinicId={clinic.id} /> : <EntNotice />)}
        {tab === 'insurance' &&
          (isEnterprise ? <InsuranceTab clinicId={clinic.id} /> : <EntNotice />)}
...
function EntNotice() {
  return (
    <div className="rounded-md border border-amber-300 bg-amber-50 p-4 text-sm text-amber-800">
      Bu funksiya faqat <b>Enterprise (120pro)</b> tarifda mavjud. Avval "Tahrir" tabidan tarifni
      o'zgartiring.
    </div>
  );
}
  ...
  const mut = useMutation({
    mutationFn: () => {
      ...
      return api.admin.sendTenantMessage(clinic.id, {
        channels,
        plan_snapshot: clinic.current_plan ?? undefined,
        amount_uzs: amount ? Number(amount) : undefined,
    ...
    onSuccess: (r) => {
      toast.success(
        `Yuborildi — in-app: ${r.in_app ? '✓' : '—'}, telegram: ${r.telegram ? '✓' : '—'}, email: ${r.email ? '✓' : '—'}`,
      );
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap gap-4 text-sm">
        <label className="flex items-center gap-2">
          ...
          Ilova ichida (bloklovchi modal)
        </label>
        <label className="flex items-center gap-2">
          <input type="checkbox" checked={tg} onChange={(e) => setTg(e.target.checked)} /> Telegram
          (owner)
        </label>
        <label className="flex items-center gap-2">
      ...
      <div className="bg-muted/40 text-muted-foreground rounded-md p-2 text-xs">
        Klinikaga: tarif <b>{clinic.current_plan ?? '—'}</b>
        {amount ? `, ${fmt(Number(amount))} so'm` : ''}
        {payDate ? `, ${payDate}` : ''} + aloqa {contact}
      </div>
      <Button className="w-full gap-1.5" disabled={mut.isPending} onClick={() => mut.mutate()}>
...
function BranchesTab({ clinicId }: { clinicId: string }) {
  ...
  const { data } = useQuery({
    queryKey: ['adm-branches', clinicId],
    queryFn: () => api.admin.tenantBranches(clinicId),
  });
  const { data: tenants } = useQuery({
    queryKey: ['adm-tenants-all'],
    queryFn: () => api.admin.listTenants(),
  });
  const [pick, setPick] = useState('');
  const invalidate = () => qc.invalidateQueries({ queryKey: ['adm-branches', clinicId] });
  const linkMut = useMutation({
    mutationFn: () => api.admin.linkBranch(clinicId, pick),
    onSuccess: () => {
  ...
  const unlinkMut = useMutation({
    mutationFn: (bid: string) => api.admin.unlinkBranch(clinicId, bid),
    onSuccess: () => {
  ...
  const candidates = ((tenants ?? []) as Array<{ id: string; name: string }>).filter(
    (t) => !branchIds.has(t.id) && t.id !== clinicId,
  );
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-end gap-2">
        ...
        <Button disabled={!pick || linkMut.isPending} onClick={() => linkMut.mutate()}>
          <Link2 className="mr-1 h-4 w-4" /> Bog'lash
        </Button>
      </div>
          ...
          >
            ...
            {!b.is_hq && (
              <button
                ...
                onClick={() => unlinkMut.mutate(b.id)}
              >
                <Unlink className="h-3 w-3" /> ajratish
              </button>
            )}
          </div>
        ))}
        {(data?.branches ?? []).length === 0 && (
          <p className="text-muted-foreground text-sm">Filial yo'q.</p>
        )}
      </div>
    </div>
  );
...
function InsuranceTab({ clinicId }: { clinicId: string }) {
  const qc = useQueryClient();
  const { data } = useQuery({
    queryKey: ['adm-insurance', clinicId],
    queryFn: () => api.admin.tenantInsurance(clinicId),
  });
  const { data: providers } = useQuery({
    queryKey: ['adm-ins-providers'],
    queryFn: () => api.admin.listInsuranceProviders(),
  });
  ...
  const mut = useMutation({
    mutationFn: () =>
      api.admin.linkInsurance(clinicId, {
        name: name.trim(),
        provider_id: providerId || undefined,
        copay_percent: Number(copay) || 0,
      }),
    onSuccess: () => {
      toast.success("Sug'urta bog'landi");
      setName('');
      setProviderId('');
      qc.invalidateQueries({ queryKey: ['adm-insurance', clinicId] });
    },
    onError: (e: Error) => toast.error(e.message),
  });
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-3 gap-2">
        <div className="space-y-1.5">
          <Label>Nomi</Label>
          <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Apex paket" />
        </div>
        <div className="space-y-1.5">
      ...
      <div className="space-y-1">
        {(data ?? []).map((c) => (
          ...
          >
            ...
            <span className="text-muted-foreground text-xs">copay {c.copay_percent}%</span>
          </div>
        ))}
        {(data ?? []).length === 0 && (
          <p className="text-muted-foreground text-sm">Shartnoma yo'q.</p>
        )}
      </div>
    </div>
  );
...
function RemindersTab({ clinicId }: { clinicId: string }) {
  const qc = useQueryClient();
  const { data } = useQuery({
    queryKey: ['adm-reminders', clinicId],
    queryFn: () => api.admin.tenantReminders(clinicId),
  });
  const [note, setNote] = useState('');
  const invalidate = () => qc.invalidateQueries({ queryKey: ['adm-reminders', clinicId] });
  const addMut = useMutation({
    mutationFn: () => api.admin.addReminder(clinicId, note.trim()),
    onSuccess: () => {
      setNote('');
      invalidate();
    },
  ...
  const doneMut = useMutation({
    mutationFn: (rid: string) => api.admin.doneReminder(clinicId, rid),
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-1">
        {(data ?? []).map((r) => (
          <div
            key={r.id}
            className={`flex items-center justify-between rounded border px-2 py-1.5 text-sm ${r.is_done ? 'opacity-50' : ''}`}
          >
        ...
        {(data ?? []).length === 0 && (
          <p className="text-muted-foreground text-sm">Eslatma yo'q.</p>
        )}
      </div>
    </div>
  );
  ...
  const mut = useMutation({
    mutationFn: async () => {
      if (name.trim() && name.trim() !== clinic.name)
        await api.admin.updateTenant(clinic.id, { name: name.trim() });
...
function DmedTab({ clinicId }: { clinicId: string }) {
  ...
  if (loaded && loaded.status !== 'not_configured' && !clientId && loaded.client_id)
    formInit(loaded);
  ...
  const saveMut = useMutation({
    mutationFn: () => {
      if (!clientId.trim() || !fhirBase.trim() || !facilityCode.trim())
        throw new Error('client_id, FHIR URL va muassasa kodi majburiy');
      return api.admin.saveDmedConnection(clinicId, {
        client_id: clientId.trim(),
        fhir_base_url: fhirBase.trim(),
    ...
    onSuccess: () => {
      toast.success("DMED ma'lumotlari saqlandi");
      setSecret('');
      inv();
    },
    onError: (e: Error) => toast.error(e.message),
  ...
  const inviteMut = useMutation({
    mutationFn: () => api.admin.inviteDmed(clinicId),
    onSuccess: () => {
  ...
  const activateMut = useMutation({
    mutationFn: () => api.admin.activateDmed(clinicId),
    onSuccess: () => {
      toast.success('Darhol faollashtirildi');
      inv();
    },
    onError: (e: Error) => toast.error(e.message),
  });
  const disconnectMut = useMutation({
    mutationFn: () => api.admin.disconnectDmed(clinicId),
    onSuccess: () => {
  ...
  const testMut = useMutation({
    mutationFn: () => api.admin.testDmed(clinicId),
    onSuccess: (r) => toast.success(r.message ?? 'Test muvaffaqiyatli'),
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  return (
    <div className="space-y-4">
      ...
      {loaded && loaded.status !== 'not_configured' && (
        <div className="bg-muted/30 text-muted-foreground space-y-1 rounded-md p-3 text-xs">
          {loaded.invited_at && (
            <div>So'rov: {new Date(loaded.invited_at).toLocaleString('uz-UZ')}</div>
          )}
          {loaded.accepted_at && (
            <div>Qabul: {new Date(loaded.accepted_at).toLocaleString('uz-UZ')}</div>
          )}
          {loaded.declined_at && (
            <div>Rad: {new Date(loaded.declined_at).toLocaleString('uz-UZ')}</div>
          )}
          {loaded.last_sync_at && (
            <div>Oxirgi sinx: {new Date(loaded.last_sync_at).toLocaleString('uz-UZ')}</div>
          )}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1.5">
          <Label>
            Secret{' '}
            {loaded?.has_secret && (
              <span className="ml-1 text-[10px] text-emerald-600">saqlangan ✓</span>
            )}
          </Label>
          <Input
            type="password"
            ...
            placeholder={
              loaded?.has_secret ? "••••• (o'zgartirish uchun kiriting)" : 'Client secret'
            }
          />
        </div>
        <div className="col-span-2 space-y-1.5">
          ...
          <Input
            ...
            placeholder="https://dmed.health.gov.uz/fhir"
          />
        </div>
        <div className="space-y-1.5">
          <Label>Muassasa kodi (MoH)</Label>
          <Input
            value={facilityCode}
      ...
      {(auditLog ?? []).length > 0 && (
        <div className="space-y-1">
          <p className="text-muted-foreground text-xs font-medium">Amallar tarixi</p>
          {(auditLog ?? []).map((a) => (
            <div
              key={a.id}

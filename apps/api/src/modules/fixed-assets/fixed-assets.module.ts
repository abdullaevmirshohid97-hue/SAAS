...
const AssetSchema = z.object({
  ...
  category: z
    .enum(['equipment', 'computer', 'furniture', 'vehicle', 'building', 'other'])
    .optional(),
...
export class FixedAssetsService {
  ...
  async list(clinicId: string) {
    ...
    return (data ?? []).map((a) => {
      ...
      return {
        ...r,
        net_book_value_uzs: Number(r.cost_uzs) - Number(r.accumulated_depreciation_uzs),
      };
    });
  }
  ...
  async create(clinicId: string, userId: string | null, body: z.infer<typeof AssetSchema>) {
    ...
    const { data, error } = await admin
      ...
      .insert({
        ...
        acquisition_date: body.acquisition_date ?? undefined,
        cost_uzs: body.cost_uzs,
        residual_uzs: body.residual_uzs ?? 0,
    ...
    if (body.capitalize && body.cost_uzs > 0) {
      try {
        await admin.rpc('post_journal', {
          p_clinic: clinicId,
          p_type: 'asset_acquire',
          p_date: body.acquisition_date ?? new Date().toISOString().slice(0, 10),
          p_source_table: 'fixed_assets',
          p_source_id: assetId,
          p_memo: 'Asosiy vosita: ' + body.name,
          p_lines: [
            { code: '1500', debit: body.cost_uzs, credit: 0 },
            {
              code: body.payment_method === 'cash' ? '1010' : '1030',
              debit: 0,
              credit: body.cost_uzs,
            },
          ],
        });
      } catch {
        /* kapitalizatsiya GL xatosi asosiy yozuvni bloklamaydi */
      }
    }
  ...
  async runDepreciation(clinicId: string, period?: string) {
    const { data, error } = await this.supabase.admin().rpc('run_depreciation', {
      p_clinic: clinicId,
...
export class FixedAssetsCronService {
  ...
  async monthlyDepreciation(): Promise<void> {
    ...
    for (const c of (data ?? []) as Array<{ id: string }>) {
      try {
        const { data: n } = await this.supabase
          .admin()
          .rpc('run_depreciation', { p_clinic: c.id, p_period: period });
        if ((n as number) > 0) this.logger.log(`Amortizatsiya ${c.id} (${period}): ${n} ta`);
      } catch (e) {
        this.logger.warn(`Amortizatsiya ${c.id} xato: ${(e as Error).message}`);
      }
    }
  }
}

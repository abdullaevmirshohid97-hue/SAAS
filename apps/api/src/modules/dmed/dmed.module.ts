...
export class DmedService {
  ...
  async get(clinicId: string) {
    const { data } = await this.sb()
      ...
      .select(
        'id, status, client_id, fhir_base_url, facility_code, scopes, invited_at, accepted_at, declined_at, force_activated, last_sync_at, last_error, created_at, updated_at, secret_vault_id',
      )
  ...
  async save(
    ...
    if (input.secret) {
      ...
      const { data: vaultId, error: vErr } = await (
        sb as unknown as {
      ...
      if (ex?.secret_vault_id) {
        await (
          ...
          .rpc('delete_secret', { secret_id: ex.secret_vault_id })
          .catch(() => undefined);
      }
    }
  ...
  async invite(clinicId: string, actorId: string | null) {
    const { data: conn } = await this.sb()
      .from('dmed_connections')
      .select('status, client_id, fhir_base_url')
      .eq('clinic_id', clinicId)
      .maybeSingle();
    ...
    if (!c.client_id || !c.fhir_base_url)
      throw new BadRequestException('client_id va fhir_base_url majburiy');
  ...
  async forceActivate(clinicId: string, actorId: string | null) {
    ...
    if ((conn as { status: string }).status === 'active')
      throw new BadRequestException('Allaqachon faol');
    ...
    await this.audit(clinicId, 'force_activate', actorId, {
      note: 'super-admin majburiy faollashtirdi',
    });
    return { ok: true };
  }
  ...
  async test(clinicId: string) {
    const { data: conn } = await this.sb()
      .from('dmed_connections')
      .select('client_id, fhir_base_url')
      .eq('clinic_id', clinicId)
      .maybeSingle();
    if (!conn) throw new NotFoundException('DMED konfiguratsiyasi topilmadi');
  ...
  async accept(clinicId: string, userId: string) {
    const { data: conn } = await this.sb()
      ...
      .eq('clinic_id', clinicId)
      .maybeSingle();
    if (!conn || (conn as { status: string }).status !== 'invited')
      throw new BadRequestException('Faol taklif topilmadi');
...
@Module({
  ...
  providers: [DmedService, SupabaseService, { provide: DMED_CLIENT, useClass: MockDmedClient }],
  exports: [DmedService],
})

...
export class InventoryService {
  ...
  async lowStock(clinicId: string) {
    const rows = (await this.stockSummary(clinicId)) as Array<{
      item_id: string;
      name: string;
      qty_in_stock: number;
  ...
  async expiring(clinicId: string) {
    const { data } = await this.supabase
      ...
      .select(
        'id, batch_no, expiry_date, qty_remaining, unit_cost_uzs, item:inventory_items(name, unit)',
      )
      .eq('clinic_id', clinicId)
      .gt('qty_remaining', 0)
  ...
  async batches(clinicId: string, itemId?: string) {
    let q = this.supabase
      ...
      .from('inventory_batches')
      .select(
        'id, item_id, batch_no, expiry_date, qty_received, qty_remaining, unit_cost_uzs, received_at',
      )
      .eq('clinic_id', clinicId)
      .gt('qty_remaining', 0)
      .order('expiry_date', { ascending: true });
  ...
  async paySupplier(
    clinicId: string,
    userId: string | null,
    body: z.infer<typeof PaySupplierSchema>,
  ) {
    const { error } = await this.supabase
      .admin()
      .from('inventory_supplier_ledger')
      .insert({
        clinic_id: clinicId,
        supplier_id: body.supplier_id ?? null,
        entry_kind: 'payment',
        ...
        payment_method: body.payment_method ?? 'cash',
        notes: body.notes ?? "Supplierga to'lov",
        created_by: userId,
      });
    if (error) throw new Error(error.message);
    return { ok: true };

...
const ContractUpdateSchema = ContractSchema.partial().extend({
...
const SettlementSchema = z.object({
  ...
  allocations: z
    .array(z.object({ claim_id: z.string().uuid(), amount_uzs: z.number().int().positive() }))
    .min(1),
});
...
export class InsuranceService {
  ...
  async updateContract(
    ...
    id: string,
    userId: string | null,
    body: z.infer<typeof ContractUpdateSchema>,
  ) {
  ...
  computeCoverage(
    ...
    const covered = Array.isArray(contract.covered_category_ids)
      ? contract.covered_category_ids
      : [];
    const lines = items.map((it) => {
      const isCovered =
        covered.length === 0 || (it.category_id != null && covered.includes(it.category_id));
  ...
  async previewCoverage(
    ...
    if (!insurerId)
      return { applicable: false as const, insurer_total: 0, copay_total: 0, lines: [] };
    ...
    const covItems = items.map((it) => {
      const svc = map.get(it.service_id) as
        | { name_i18n?: Record<string, string>; price_uzs?: number; category_id?: string | null }
        | undefined;
      const unit =
        it.unit_price_uzs && it.unit_price_uzs > 0
          ? it.unit_price_uzs
          : Number(svc?.price_uzs ?? 0);
      ...
      return {
        ...
        amount,
        name: nm ? (nm['uz-Latn'] ?? Object.values(nm)[0]) : undefined,
      };
    });
  ...
  async createClaim(
    ...
    const { data: claim, error } = await admin
      ...
      .insert({
        ...
        claim_no,
        claim_amount_uzs: input.claim_amount_uzs,
        copay_amount_uzs: input.copay_amount_uzs,
    ...
    const itemRows = input.lines
      ...
      .map((l) => ({
        ...
        name_snapshot: l.name ?? null,
        covered_amount_uzs: l.covered,
        copay_amount_uzs: l.copay,
      }));
  ...
  async listClaims(clinicId: string, status?: string) {
    let q = this.supabase
      ...
      .from('insurance_claims')
      .select(
        '*, insurer:insurance_companies(name), provider:insurance_providers(name), patient:patients(full_name)',
      )
      .eq('clinic_id', clinicId)
  ...
  async getClaim(clinicId: string, id: string) {
    const { data } = await this.supabase
      ...
      .from('insurance_claims')
      .select(
        '*, insurer:insurance_companies(name, contract_no), provider:insurance_providers(name), patient:patients(full_name, insurance_policy_no), items:insurance_claim_items(*)',
      )
      .eq('clinic_id', clinicId)
      .eq('id', id)
      .maybeSingle();
  ...
  private async recomputeClaim(clinicId: string, claimId: string) {
    ...
    const { data: claim } = await admin
      .from('insurance_claims')
      .select('claim_amount_uzs, status')
      .eq('id', claimId)
      .single();
    ...
    let paid = 0,
      total = 0;
    const rows = (allocs ?? []) as unknown as Array<{
      amount_uzs: number;
      settlement: { method: string } | { method: string }[] | null;
    }>;
    for (const a of rows) {
    ...
    const status =
      (claim as { status: string }).status === 'denied'
        ? 'denied'
        : total >= claimAmt
          ? 'paid'
          : total > 0
  ...
  async createSettlement(
    ...
    const { data: settle, error } = await admin
      ...
      .insert({
        ...
        amount_uzs: total,
        method: body.method,
        settled_at: body.settled_at ?? undefined,
  ...
  async payClaim(
    ...
    userId: string | null,
    id: string,
    body: { amount_uzs?: number; method?: string },
  ) {
    const { data: claim } = await this.supabase
      .admin()
      .from('insurance_claims')
      .select('claim_amount_uzs, paid_amount_uzs, insurer_id')
      .eq('clinic_id', clinicId)
      .eq('id', id)
      .single();
    const c = claim as {
    ...
    return this.createSettlement(clinicId, userId, {
      insurer_id: c.insurer_id ?? undefined,
  ...
  async insurerAging(clinicId: string, asOf: string) {
    const { data, error } = await this.supabase
      .admin()
      .rpc('insurer_aging', { p_clinic: clinicId, p_as_of: asOf });
    if (error) throw new Error(error.message);
...
class InsuranceController {
  ...
  updateContract(
    ...
    return this.svc.updateContract(
      ...
      id,
      u.userId ?? null,
      ContractUpdateSchema.parse(body),
    );
  }

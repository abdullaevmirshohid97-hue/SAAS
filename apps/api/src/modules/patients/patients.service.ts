  ...
  async timeline(clinicId: string, patientId: string) {
    ...
    const [
      ...
      admin
        .from('appointments')
        .select(
          'id, scheduled_at, status, service_name_snapshot, doctor:profiles!doctor_id(full_name)',
        )
        .in('clinic_id', scope)
      ...
      admin
        .from('prescriptions')
        .select(
          'id, rx_number, status, created_at, items:prescription_items(medication_name_snapshot, quantity)',
        )
        .in('clinic_id', scope)
        .eq('patient_id', patientId)
        .order('created_at', { ascending: false })
      ...
      admin
        .from('pharmacy_sales')
        .select(
          'id, total_uzs, payment_method, created_at, items:pharmacy_sale_items(name_snapshot, quantity, subtotal_uzs)',
        )
        .in('clinic_id', scope)
        .eq('patient_id', patientId)
        .order('created_at', { ascending: false })
    ...
    for (const a of (appts.data ?? []) as Array<Record<string, unknown>>) {
      push({
        ...
        title: (a.service_name_snapshot as string) || 'Tashrif',
        subtitle: name(a.doctor),
    ...
    for (const n of (notes.data ?? []) as Array<Record<string, unknown>>) {
      push({
        id: `note-${n.id}`,
        type: 'note',
        date: (n.signed_at as string) || (n.created_at as string),
        title: (n.diagnosis_text as string) || 'Klinik qayd',
        subtitle: name(n.author),
        status: n.is_final ? 'final' : 'draft',
        ref_id: n.id as string,
        module: 'doctor',
        icd: n.diagnosis_code
          ? { code: n.diagnosis_code as string, name: (n.diagnosis_text as string) ?? '' }
          : undefined,
        details: {
    ...
    for (const l of (labOrders.data ?? []) as Array<Record<string, unknown>>) {
      ...
      const abnormal = items.some((it) =>
        ((it.results as Array<{ is_abnormal?: boolean }>) ?? []).some((r) => r.is_abnormal),
      );
      push({
        id: `lab-${l.id}`,
        type: 'lab',
        date: (l.completed_at as string) || (l.created_at as string),
        title: 'Laboratoriya',
    ...
    for (const d of (diagnostics.data ?? []) as Array<Record<string, unknown>>) {
      ...
      const atts = res.flatMap(
        (r) => (r.attachments as Array<{ name?: string; url?: string }>) ?? [],
      );
      push({
        id: `diag-${d.id}`,
        type: 'diagnostic',
        date: (d.created_at as string) || (d.scheduled_at as string),
        title: (d.name_snapshot as string) || 'Diagnostika',
        status: d.status as string,
        ref_id: d.id as string,
        module: 'diagnostics',
        attachments: atts
          .map((a) => ({ name: a.name ?? 'Fayl', url: a.url ?? '' }))
          .filter((a) => a.url),
    ...
    for (const p of (prescriptions.data ?? []) as Array<Record<string, unknown>>) {
      push({
        ...
        title: `Retsept ${p.rx_number ?? ''}`.trim(),
    ...
    for (const s of (pharmacy.data ?? []) as Array<Record<string, unknown>>) {
      push({
        ...
        title: 'Dorixona',
        amount_uzs: Number(s.total_uzs ?? 0),
    ...
    for (const t of (tx.data ?? []) as Array<Record<string, unknown>>) {
      push({
        ...
        status: t.is_void ? 'void' : (t.kind as string),
    ...
    for (const s of (stays.data ?? []) as Array<Record<string, unknown>>) {
      ...
      push({
        id: `stay-${s.id}`,
        type: 'inpatient',
        date: s.admitted_at as string,
        title: 'Statsionar',
        subtitle: room?.number ? `Palata ${room.number}` : null,
        status: s.status as string,
        amount_uzs: Number(s.total_cost_uzs ?? 0),
        ref_id: s.id as string,
        module: 'inpatient',
    ...
    for (const v of (vitals.data ?? []) as Array<Record<string, unknown>>) {
      push({
        ...
        title: 'Vital belgilar',
        subtitle: name(v.recorder),
    ...
    for (const r of (referrals.data ?? []) as Array<Record<string, unknown>>) {
      push({
        ...
        title: (r.service_name_snapshot as string) || "Yo'naltirish",
        status: r.status as string,
        ref_id: r.id as string,
        module: 'reception',
        details: { kind: r.kind, notes: r.notes },
      });
    }

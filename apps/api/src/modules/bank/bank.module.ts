...
export class BankService {
  ...
  async autoMatch(clinicId: string, accountId: string) {
    const { data, error } = await this.supabase
      .admin()
      .rpc('bank_auto_match', { p_clinic: clinicId, p_account: accountId });
    if (error) throw new Error(error.message);
  ...
  async reconciliation(clinicId: string, accountId: string) {
    ...
    const { data: gl } = await admin.rpc('gl_account_activity', {
      p_clinic: clinicId,
      p_from: '2000-01-01',
      p_to: new Date().toISOString().slice(0, 10),
    });
    const glRow = ((gl ?? []) as Array<{ code: string; debit: number; credit: number }>).find(
      (r) => r.code === glCode,
    );
...
class BankController {
  ...
  status(
    ...
    @Param('id') id: string,
    @Body() body: { status: 'pending' | 'matched' | 'ignored'; journal_id?: string },
  ) {
    if (!u.clinicId) throw new ForbiddenException();

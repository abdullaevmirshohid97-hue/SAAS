...
export class PatientPortalService {
  ...
  async createBooking(portalUserId: string, input: { slot_id: string; reason?: string }) {
    ...
    if (new Date(slot.starts_at) < new Date())
      throw new BadRequestException("Bu vaqt o'tib ketgan");
  ...
  async getQueueStatus(bookingId: string) {
    ...
    return {
      ...
      estimated_wait_min: ['pending', 'confirmed'].includes(booking.status)
        ? estimatedWaitMin
        : null,
      slot: booking.slot,
  ...
  async listMyAppointmentRequests(portalUserId: string) {
    ...
    return rows.map((r) => ({
      ...r,
      doctor_name: r.doctor_id ? (docById.get(r.doctor_id) ?? null) : null,
    }));
  }
  ...
  async treatmentStatus(portalUserId: string) {
    ...
    const [{ data: clinics }, { data: stays }] = await Promise.all([
      ...
      admin
        ...
        .select(
          'id, patient_id, clinic_id, admitted_at, status, room:rooms(number, floor, name_i18n, type)',
        )
  ...
  async inpatientNurseCall(portalUserId: string, stayId: string, note?: string) {
    ...
    const { data: stay } = await admin
      .from('inpatient_stays')
      .select(
        'id, clinic_id, patient_id, status, room:rooms(number, floor), patient:patients(phone, full_name)',
      )
      .eq('id', stayId)
      .eq('status', 'admitted')
      .maybeSingle();
    ...
    const st = stay as unknown as {
      ...
      patient_id: string;
      room:
        | { number: string; floor: number | null }
        | { number: string; floor: number | null }[]
        | null;
      patient:
        | { phone: string | null; full_name: string }
        | { phone: string | null; full_name: string }[]
        | null;
    };
    ...
    const roomLabel = room
      ? `Xona ${room.number}${room.floor != null ? `, ${room.floor}-qavat` : ''}`
      : 'Xona —';
  ...
  async getMedicalRecords(portalUserId: string) {
    ...
    const { data: pu } = await admin
      ...
      .maybeSingle();
    if (!pu?.phone)
      return { patients: [], diagnoses: [], labs: [], diagnostics: [], prescriptions: [] };
    ...
    const [
      ...
      admin
        ...
        .select(
          'id, patient_id, clinic_id, author_id, diagnosis_code, diagnosis_text, soap_assessment, soap_plan, is_final, signed_at, created_at',
        )
        .in('patient_id', patientIds)
        .order('created_at', { ascending: false }),
      admin
        .from('prescriptions')
        .select(
          'id, patient_id, clinic_id, doctor_id, rx_number, diagnosis_code, diagnosis_text, instructions, status, created_at',
        )
        .in('patient_id', patientIds)
        .order('created_at', { ascending: false }),
      admin
        .from('lab_orders')
        .select(
          'id, patient_id, clinic_id, status, urgency, created_at, completed_at, items:lab_order_items(id, name_snapshot, status, results:lab_results(value, unit, reference_range, flag, is_abnormal, interpretation, reported_at))',
        )
        .in('patient_id', patientIds)
        .order('created_at', { ascending: false }),
      admin
        ...
        .select(
          'id, patient_id, clinic_id, name_snapshot, status, created_at, results:diagnostic_results(findings, impression, is_final, reported_at)',
        )
        .in('patient_id', patientIds)
        .order('created_at', { ascending: false }),
    ]);
  ...
  async getReviews(clinicSlug: string, page = 1) {
    ...
    const { data, count, error } = await this.supabase
      ...
      .from('clinic_reviews')
      .select(
        'id,rating,comment,helpful_count,reply_text,replied_at,is_verified,created_at,portal_user_id',
        { count: 'exact' },
      )
      .eq('clinic_id', clinic.id)
      .eq('is_hidden', false)

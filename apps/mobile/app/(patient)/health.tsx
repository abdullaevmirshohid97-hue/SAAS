  ...
  return (
    <View className="flex-1 bg-white dark:bg-black" style={{ paddingTop: insets.top + 12 }}>
      ...
      <Text className="px-4 text-sm text-gray-500 dark:text-gray-400">
        Shifokor qo'ygan tashxis va natijalar
      </Text>
      ...
      <View className="mt-3 flex-row gap-2 px-4">
        {tabs.map((t) => (
          ...
          >
            <Text
              className={`text-xs font-semibold ${tab === t.key ? 'text-white' : 'text-gray-600 dark:text-gray-300'}`}
            >
              {t.label}
              {t.count > 0 ? ` (${t.count})` : ''}
            </Text>
                        ...
                        {it.results.length === 0 ? (
                          ...
                          it.results.map((r, j) => (
                            <View key={j} className="mt-1 flex-row items-center justify-between">
                              <Text
                                className={`text-sm font-semibold ${r.is_abnormal ? 'text-red-600' : 'text-green-700'}`}
                              >
                                {r.value}
                                {r.unit ? ` ${r.unit}` : ''}
                              </Text>
          ...
          {tab === 'diagnostics' &&
            (diagnostics.length === 0 ? (
              ...
              diagnostics.map((o) => (
                ...
                >
                  <View className="flex-row items-center justify-between">
                    <Text className="flex-1 font-semibold dark:text-white">
                      {o.name || 'Tekshiruv'}
                    </Text>
                    <StatusBadge status={o.status} />
                  </View>

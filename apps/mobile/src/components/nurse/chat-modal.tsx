...
export function ChatModal({ task, onClose }: { task: ChatTask; onClose: () => void }) {
  ...
  const send = useMutation({
    ...
    onSuccess: () => {
      setText('');
      qc.invalidateQueries({ queryKey: ['nurse', 'chat', task.id] });
    },
        ...
        <ScrollView className="flex-1 p-4" contentContainerStyle={{ gap: 8 }}>
          {(data ?? []).map((m) => (
            ...
            >
              <Text
                className={
                  m.sender_kind === 'nurse' ? 'text-white' : 'text-gray-800 dark:text-gray-100'
                }
              >
                {m.body}
              </Text>
          ...
          {(data ?? []).length === 0 ? (
            <Text className="mt-8 text-center text-gray-400">Hali xabar yo'q</Text>
          ) : null}
        </ScrollView>
        <View
          className="flex-row items-center gap-2 border-t border-gray-100 p-3 dark:border-gray-800"

...
const FILTERS: Array<{ id: 'all' | TimelineEvent['type']; label: string }> = [
  ...
  { id: 'visit', label: 'Tashrif' },
  { id: 'note', label: 'Qayd' },
  { id: 'lab', label: 'Tahlil' },
  { id: 'diagnostic', label: 'Diagnostika' },
  { id: 'prescription', label: 'Retsept' },
...
const yearOf = (v: string) =>
  Number.isNaN(new Date(v).getTime()) ? '—' : new Date(v).getFullYear().toString();
...
function EventDetails({ e }: { e: TimelineEvent }) {
  ...
  if (e.type === 'lab') {
    ...
    if (e.abnormal)
      rows.push(
        <span key="ab" className="font-semibold text-red-600 dark:text-red-400">
          ⚠ Normadan chetlagan natija
        </span>,
      );
  }
  if (e.type === 'diagnostic' && d.impression)
    rows.push(<span key="imp">{String(d.impression)}</span>);
  if (e.type === 'prescription') {
    const items =
      (d.items as Array<{ medication_name_snapshot?: string; quantity?: number }>) ?? [];
    rows.push(
      <span key="rx">
        {items
          .map(
            (it) => `${it.medication_name_snapshot ?? ''}${it.quantity ? ` ×${it.quantity}` : ''}`,
          )
          .filter(Boolean)
          .join(', ')}
      </span>,
  ...
  if (e.type === 'pharmacy') {
    ...
    rows.push(
      <span key="ph">
        {items
          .map((it) => `${it.name_snapshot ?? ''}${it.quantity ? ` ×${it.quantity}` : ''}`)
          .filter(Boolean)
          .join(', ')}
      </span>,
    );
  ...
  if (typeof e.amount_uzs === 'number' && e.amount_uzs !== 0)
    rows.push(
      <span key="amt" className="font-semibold">
        {fmtMoney(e.amount_uzs)} so'm
      </span>,
    );
...
export function PatientTimeline({ events }: { events: TimelineEvent[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return events.filter((e) => {
      ...
      const hay =
        `${e.title} ${e.subtitle ?? ''} ${e.icd?.code ?? ''} ${e.icd?.name ?? ''} ${JSON.stringify(e.details ?? {})}`.toLowerCase();
    ...
    <div className="space-y-4">
      ...
      <div className="relative max-w-md">
        ...
        <Input
          ...
          placeholder="Qidirish: Diabetes, MRT, retsept…"
          className="pl-9"
        />
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-6">
          {groups.map(([year, items]) => (
            <div key={year}>
              <div className="bg-background/95 text-muted-foreground sticky top-0 z-10 mb-2 py-1 text-sm font-bold backdrop-blur">
                {year}
              </div>
                ...
                {items.map((e) => {
                  ...
                  return (
                    <li key={e.id} className="relative">
                      <span
                        className={`ring-background absolute -left-[34px] flex h-7 w-7 items-center justify-center rounded-full ring-4 ${meta.cls}`}
                      >
                      ...
                      <div className="bg-card rounded-lg border p-3">
                        <div className="flex items-start justify-between gap-2">
                          <div className="min-w-0">
                            ...
                            {e.subtitle && (
                              <div className="text-muted-foreground text-xs">{e.subtitle}</div>
                            )}
                          </div>

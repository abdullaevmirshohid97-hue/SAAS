...
export function DayZReportDialog({ onClose }: { onClose: () => void }) {
  ...
  const handlePrint = () => {
    ...
    const rows = data.by_method
      .map(
        (m) =>
          `<tr><td>${esc(ml(m.method))}</td><td class="r">${fmt(m.revenue_uzs)}</td><td class="r">${m.refund_uzs ? '−' + fmt(m.refund_uzs) : '—'}</td><td class="r b">${fmt(m.net_uzs)}</td></tr>`,
      )
      .join('');
      ...
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Receipt className="text-primary h-5 w-5" /> Kunlik Z-hisobot
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="flex items-center gap-2">
          ...
          {data && (
            <span className="text-muted-foreground text-sm">
              {data.shifts.length} smena
              {data.cash.open_shifts_count > 0 ? ` (${data.cash.open_shifts_count} ochiq)` : ''}
            </span>
          )}
        </div>

        {isLoading || !data ? (
          <div className="text-muted-foreground py-8 text-center text-sm">Yuklanmoqda…</div>
        ) : (
          <div className="space-y-4">
            ...
            <div className="rounded-lg border p-3 text-sm">
              ...
              <Row
                label="Maosh to'lovi"
                value={`−${fmt(data.totals.payroll_uzs)} so'm`}
                tone="rose"
              />
              {data.totals.pharmacy_paid_uzs > 0 && (
                <Row
                  label="Dorixona (to'langan)"
                  value={`${fmt(data.totals.pharmacy_paid_uzs)} so'm`}
                />
              )}
              {data.transfers_uzs > 0 && (
                <Row
                  label="Inkassatsiya (seyfga)"
                  value={`${fmt(data.transfers_uzs)} so'm`}
                  tone="sky"
                />
              )}
              <div className="mt-1.5 flex items-center justify-between border-t pt-2 text-base font-bold">
                <span>SOF NATIJA</span>
                <span className={data.totals.net_uzs >= 0 ? 'text-emerald-700' : 'text-rose-700'}>
                  {fmt(data.totals.net_uzs)} so'm
                </span>
              </div>
            </div>

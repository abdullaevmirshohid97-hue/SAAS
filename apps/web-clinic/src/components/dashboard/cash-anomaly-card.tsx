    ...
    <Card className={cn(highCount > 0 && 'border-rose-300 bg-rose-50/30')}>
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle className="flex items-center gap-2 text-base">
          <ShieldAlert
            className={cn('h-4 w-4', highCount > 0 ? 'text-rose-600' : 'text-muted-foreground')}
          />
          Kassa anomaliyalar
        </CardTitle>
        {anomalies.length > 0 && (
          <span className="text-muted-foreground text-xs">{anomalies.length} ta topildi</span>
        )}
      </CardHeader>
      <CardContent>
        {isLoading ? (
          ...
          <ul className="space-y-2">
            {anomalies.slice(0, 5).map((s) => {
              ...
              return (
                <li
                  key={s.id}
                  className={cn('flex items-center gap-2 rounded-md border p-2 text-sm', meta.tone)}
                >
                  <Icon className="h-4 w-4 shrink-0" />
                  <div className="min-w-0 flex-1">
                    <div className="font-medium">{s.operator?.full_name ?? 'Operator'}</div>

...
export function FixedAssetsPage() {
  ...
  const depMut = useMutation({
    ...
    onSuccess: (r) => {
      toast.success(`Amortizatsiya: ${r.posted} ta vosita hisoblandi`);
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        description="MRT/KT/UZI/kompyuter… registri + amortizatsiya (Dr 5300 / Cr 1590)."
      />

      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex gap-4 text-sm">
          <span>
            Jami xarid: <b>{fmt(totalCost)}</b>
          </span>
          <span>
            Qoldiq qiymat (NBV): <b className="text-emerald-600">{fmt(totalNbv)}</b>
          </span>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => depMut.mutate()} disabled={depMut.isPending}>
            <Calculator className="mr-1.5 h-4 w-4" /> Amortizatsiya hisoblash
          </Button>
          <Button onClick={() => setAddOpen(true)}>
            <Plus className="mr-1.5 h-4 w-4" /> Yangi vosita
          </Button>
        </div>
      </div>
      ...
      {(assets ?? []).length === 0 ? (
        <EmptyState
          title="Vosita yo'q"
          description="«Yangi vosita» bilan asosiy vosita qo'shing."
        />
      ) : (
        <Card>
            ...
            <table className="w-full text-sm">
              <thead className="text-muted-foreground border-b text-left text-xs">
                <tr>
                  ...
                  <th className="text-right">Amortizatsiya</th>
                  <th className="text-right">Qoldiq (NBV)</th>
                  <th>Holat</th>
                  ...
                  return (
                    <tr key={a.id} className="border-b last:border-0">
                      <td className="p-3 font-medium">
                        {a.name}
                        {a.location ? (
                          <span className="text-muted-foreground ml-1 text-xs">· {a.location}</span>
                        ) : null}
                      </td>
                      ...
                      <td className="text-right tabular-nums text-rose-600">
                        {fmt(a.accumulated_depreciation_uzs)}
                      </td>
                      <td className="text-right font-semibold tabular-nums">
                        {fmt(a.net_book_value_uzs)}
                      </td>
                      <td>
...
function AddAssetDialog({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  const mut = useMutation({
    mutationFn: () =>
      api.fixedAssets.create({
        name: name.trim(),
        category,
        cost_uzs: Number(cost || 0),
        residual_uzs: Number(residual || 0),
        useful_life_months: Number(life || 60),
        location: location || undefined,
        capitalize,
      }),
    onSuccess: () => {
      toast.success("Vosita qo'shildi");
      onDone();
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  const monthly =
    Number(cost || 0) > 0 && Number(life || 0) > 0
      ? Math.round((Number(cost) - Number(residual || 0)) / Number(life))
      : 0;
    ...
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Amortizatsiya straight-line: (xarid − qoldiq) / muddat.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <label className="flex flex-1 flex-col gap-1 text-xs">
              ...
              >
                <option value="equipment">Uskuna</option>
          ...
          {monthly > 0 && (
            <div className="bg-muted/40 rounded-md p-2 text-xs">
              Oylik amortizatsiya ≈ <b>{fmt(monthly)}</b> so'm
            </div>
          )}
          ...
          <Button
            className="w-full"
            disabled={!name.trim() || !(Number(cost) > 0) || mut.isPending}
            onClick={() => mut.mutate()}
          >
            Saqlash
          </Button>

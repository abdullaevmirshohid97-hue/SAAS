...
const StatusBadge = ({ s }: { s: string }) => {
  ...
  return (
    <Badge className={`text-[10px] ${st.cls}`} variant="secondary">
      {st.label}
    </Badge>
  );
};
...
async function downloadPoPdf(poId: string) {
  ...
  const rows = o.items
    .map(
      (it, i) =>
        `<tr><td>${i + 1}</td><td>${escapeHtml(it.name_snapshot)}</td>` +
...
export function ProcurementPage() {
  ...
  const { data: orders } = useQuery({
    queryKey: ['po-orders'],
    queryFn: () => api.procurement.orders(),
  });
  const { data: reorder } = useQuery({
    queryKey: ['po-reorder'],
    queryFn: () => api.procurement.reorderSuggestions(),
  });
  ...
  const approveMut = useMutation({
    mutationFn: (id: string) => api.procurement.approve(id),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Xaridlar (Procurement)"
        description="Talab → tasdiq → buyurtma → qabul → invoice. Auto-reorder + 3-way match."
      />
      ...
      <Tabs defaultValue="orders">
        <TabsList>
          ...
          <TabsTrigger value="requisitions">
            <ClipboardList className="mr-1 h-3.5 w-3.5" /> Talablar
          </TabsTrigger>
          <TabsTrigger value="invoices">
            <Receipt className="mr-1 h-3.5 w-3.5" /> Invoyslar
          </TabsTrigger>
          <TabsTrigger value="reorder">
            ⚠️ Auto-reorder{(reorder?.length ?? 0) > 0 ? ` (${reorder?.length})` : ''}
          </TabsTrigger>
          <TabsTrigger value="settings">
            <SettingsIcon className="mr-1 h-3.5 w-3.5" /> Sozlama
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="orders">
          {(orders ?? []).length === 0 ? (
            <EmptyState
              title="Buyurtma yo'q"
              description="Talab tasdiqlang yoki auto-reorder'dan yarating."
            />
          ) : (
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-muted-foreground text-sm">
                          {po.supplier?.name ?? "Supplier yo'q"}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          variant="ghost"
                          title="PDF"
                          onClick={() => downloadPoPdf(po.id).catch((e) => toast.error(String(e)))}
                        >
                        ...
                        {(po.status === 'approved' || po.status === 'partial') && (
                          <Button size="sm" onClick={() => setReceivePo(po)}>
                            <PackageCheck className="mr-1 h-3.5 w-3.5" /> Qabul
                          </Button>
                        )}
                      </div>
                    </div>
                    <div className="text-muted-foreground mt-2 space-y-0.5 text-xs">
                      {po.items.map((it) => (
                        <div key={it.id} className="flex justify-between">
                          ...
                          <span>
                            {it.qty_received}/{it.qty_ordered} × {fmt(it.unit_cost_uzs)}
                          </span>
                        </div>
                      ))}
                    </div>
...
function RequisitionsTab() {
  ...
  const { data: reqs } = useQuery({
    queryKey: ['po-reqs'],
    queryFn: () => api.procurement.requisitions(),
  });
  ...
  const invalidate = () => {
    qc.invalidateQueries({ queryKey: ['po-reqs'] });
    qc.invalidateQueries({ queryKey: ['po-orders'] });
  };
  ...
  const createMut = useMutation({
    ...
    onSuccess: (r) => {
      toast.success(`Talab yaratildi: ${r.req_no}`);
      setLines([]);
      setNote('');
      invalidate();
    },
  ...
  const approveMut = useMutation({
    mutationFn: (id: string) => api.procurement.approveRequisition(id),
    onSuccess: (r) => {
      toast.success(`PO yaratildi: ${r.po_no}`);
      invalidate();
    },
    onError: (e: Error) => toast.error(e.message),
  });
  const rejectMut = useMutation({
    mutationFn: (id: string) => api.procurement.rejectRequisition(id),
    onSuccess: () => {
                  ...
                  {r.status === 'requested' && (
                    <div className="flex items-center gap-2">
                      <Button size="sm" variant="outline" onClick={() => approveMut.mutate(r.id)}>
                        <Check className="mr-1 h-3.5 w-3.5" /> Tasdiq → PO
                      </Button>
...
function InvoicesTab({ orders }: { orders: PO[] }) {
  ...
  const { data: invoices } = useQuery({
    queryKey: ['po-invoices'],
    queryFn: () => api.procurement.invoices(),
  });
  ...
  const createMut = useMutation({
    mutationFn: () =>
      api.procurement.createInvoice({
        invoice_no: invNo.trim(),
        amount_uzs: Number(amount || 0),
        po_id: poId || undefined,
      }),
    onSuccess: () => {
      toast.success('Invoys saqlandi');
      setInvNo('');
      setAmount('');
      setPoId('');
      qc.invalidateQueries({ queryKey: ['po-invoices'] });
    },
    onError: (e: Error) => toast.error(e.message),
  });
      ...
      <Card>
        <CardContent className="space-y-3 p-4">
          ...
          <div className="flex flex-wrap items-end gap-2">
            ...
            <Button
              disabled={!invNo.trim() || createMut.isPending}
              onClick={() => createMut.mutate()}
            >
...
function MatchDialog({ poId, onClose }: { poId: string; onClose: () => void }) {
  const { data: m, isLoading } = useQuery({
    ...
    queryFn: () => api.procurement.match(poId),
  });
  return (
          ...
          <div className="space-y-3">
              ...
              <tbody>
                {m.lines.map((l, i) => (
                  <tr key={i} className="border-b last:border-0">
                    ...
                    <td
                      className={`text-right font-medium ${l.qty_variance < 0 ? 'text-amber-600' : l.qty_variance > 0 ? 'text-rose-600' : 'text-emerald-600'}`}
                    >
                      {l.qty_variance > 0 ? '+' : ''}
                      {l.qty_variance}
                    </td>
            ...
            <div className="grid grid-cols-3 gap-2 text-sm">
              <div className="rounded-md border p-2">
                <div className="text-muted-foreground text-xs">Buyurtma</div>
                <div className="font-semibold">{fmt(m.totals.ordered_uzs)}</div>
              </div>
              <div className="rounded-md border p-2">
                <div className="text-muted-foreground text-xs">Qabul</div>
                <div className="font-semibold">{fmt(m.totals.received_uzs)}</div>
              </div>
              <div className="rounded-md border p-2">
                <div className="text-muted-foreground text-xs">Invoys</div>
                <div className="font-semibold">{fmt(m.totals.invoiced_uzs)}</div>
              </div>
            </div>
            <div
              className={`rounded-md p-2 text-sm ${m.invoice_vs_received_uzs === 0 ? 'bg-emerald-500/10 text-emerald-700' : 'bg-rose-500/10 text-rose-700'}`}
            >
...
function SettingsTab() {
  ...
  const { data: s } = useQuery({
    ...
    queryFn: () => api.procurement.settings(),
  });
  const mut = useMutation({
    mutationFn: (body: { auto_reorder_enabled?: boolean; reorder_hour?: number }) =>
      api.procurement.updateSettings(body),
    onSuccess: () => {
      toast.success('Saqlandi');
      qc.invalidateQueries({ queryKey: ['po-settings'] });
    },
    onError: (e: Error) => toast.error(e.message),
  });
  return (
    <Card>
      <CardContent className="space-y-4 p-4">
        ...
        <p className="text-muted-foreground text-xs">
          Yoqilsa, har kuni ertalab zaxirasi reorder darajasidan past dorilar uchun avtomatik{' '}
          <b>qoralama PO</b> yaratiladi (oxirgi yetkazib beruvchi bo'yicha). Ochiq buyurtmadagi
          dorilar takrorlanmaydi.
        </p>
        <label className="flex items-center gap-2 text-sm">
          <input
            type="checkbox"
            checked={!!s?.auto_reorder_enabled}
            onChange={(e) => mut.mutate({ auto_reorder_enabled: e.target.checked })}
          />
        ...
        <label className="flex items-center gap-2 text-xs">
          ...
          <Input
            className="h-8 w-20"
            defaultValue={String(s?.reorder_hour ?? 6)}
            onBlur={(e) => {
              const h = Number(e.target.value);
              if (h >= 0 && h <= 23) mut.mutate({ reorder_hour: h });
            }}
          />
        </label>
      </CardContent>
...
function ReorderTab({
  ...
  const createMut = useMutation({
    ...
    onSuccess: (r) => {
      toast.success(`Buyurtma yaratildi: ${r.po_no}`);
      setRows({});
      onCreated();
    },
    onError: (e: Error) => toast.error(e.message),
  ...
  if (suggestions.length === 0)
    return <EmptyState title="Zaxira yetarli" description="Reorder darajasidan past dori yo'q." />;
    ...
    <Card>
      <CardContent className="p-4">
        <div className="mb-3 flex items-center gap-2 text-sm text-amber-700">
          <AlertTriangle className="h-4 w-4" /> Reorder darajasidan past {suggestions.length} ta
          dori
        </div>
        <div className="space-y-2">
          {suggestions.map((s) => {
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    setRows((p) => ({
                      ...p,
                      [s.medication_id]: {
                        ...get(s.medication_id, s.suggested_qty),
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-muted-foreground text-xs">
                    Zaxira: {s.qty_in_stock} · Reorder: {s.reorder_level}
                  </div>
                </div>
        ...
        <Button
          className="mt-3"
          disabled={!anySel || createMut.isPending}
          onClick={() => createMut.mutate()}
        >
...
function ReceiveDialog({
  ...
  const [qty, setQty] = useState<Record<string, string>>(() =>
    Object.fromEntries(
      po.items.map((it) => [it.id, String(Math.max(0, it.qty_ordered - it.qty_received))]),
    ),
  );
  ...
  const mut = useMutation({
    mutationFn: () => {
      const items = po.items
        ...
        .map((it) => ({
          medication_id: it.medication_id as string,
          quantity: Number(qty[it.id]),
          unit_cost_uzs: it.unit_cost_uzs,
        }));
      if (items.length === 0) throw new Error('Qabul miqdori kiritilmadi');
      return api.procurement.receive(po.id, {
        items,
        paid_uzs: paid ? Number(paid) : undefined,
    ...
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Qabul qilingan miqdorni kiriting. Zaxiraga kiritiladi va GL'ga post qilinadi.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-2">
          {po.items.map((it) => (
        ...
        <div className="mt-2 flex flex-wrap items-end gap-3 border-t pt-3">
          ...
          <Button className="ml-auto" disabled={mut.isPending} onClick={() => mut.mutate()}>
            Qabul qilish
          </Button>
        </div>
      </DialogContent>

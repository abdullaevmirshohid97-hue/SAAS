  ...
  return (
    <Card className={cn('border-l-4', tone)}>
      <CardContent className="space-y-3 p-4">
        ...
        <div className="bg-muted/40 rounded-md p-2 text-xs">
          <div className="flex items-start gap-1.5">
            ...
            <div className="flex-1">
              <div>{req.address}</div>
              {req.address_notes && (
                <div className="text-muted-foreground mt-0.5">{req.address_notes}</div>
              )}
        ...
        {req.notes && <div className="text-muted-foreground text-xs">📝 {req.notes}</div>}

        <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
          <div className="text-muted-foreground">
            Taxminiy:{' '}
            <span className="text-foreground font-medium">{fmtUZS(req.estimate_total_uzs)}</span>
            {req.quoted_price_uzs != null && (
              <>
                {' '}
                · Belgilangan:{' '}
                <span className="text-foreground font-medium">{fmtUZS(req.quoted_price_uzs)}</span>
              </>
            )}
          </div>
          {req.assigned_nurse && (
...
function AssignDialog({ req, onClose }: { req: Req; onClose: () => void }) {
  ...
  const [price, setPrice] = useState<string>(
    String(req.quoted_price_uzs ?? req.estimate_total_uzs ?? ''),
  );
  ...
  return (
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Hamshira tayinlash</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <div>
            <div className="text-xs font-medium">Bemor</div>
            <div className="text-sm">
              {req.requester_name} · {req.service}
            </div>
          </div>
          <div className="space-y-1">
            ...
            {nurses.data && nurses.data.length === 0 && (
              <p className="text-muted-foreground text-xs">
                Klinikada hamshira xodim yo'q. Avval xodim qo'shing.
              </p>
            )}
          </div>
        ...
        <DialogFooter>
          ...
          <Button disabled={!nurseId || mut.isPending} onClick={() => mut.mutate()}>
            Tayinlash
          </Button>
        </DialogFooter>
      </DialogContent>
  ...
  return (
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Chat — {req.requester_name}</DialogTitle>
        </DialogHeader>
        <div className="bg-muted/20 flex h-80 flex-col rounded-md border">
          <div className="flex-1 space-y-2 overflow-y-auto p-3">
            {(messages.data ?? []).map((m) => (
              <div
                ...
                className={cn(
                  ...
                  m.sender_kind === 'clinic'
                    ? 'bg-primary text-primary-foreground ml-auto'
                    : m.sender_kind === 'system'
                      ? 'bg-muted text-muted-foreground mx-auto text-xs'
                      : 'bg-card border',
                )}
              >
                {m.body}
            ...
            {messages.data?.length === 0 && (
              <div className="text-muted-foreground py-8 text-center text-xs">Hali xabar yo'q</div>
            )}
          </div>
          <form
            onSubmit={(e) => {
            ...
            className="bg-background flex gap-2 border-t p-2"
          >
            <Input
              value={text}

...
export function MonthClosingPage() {
  ...
  const [result, setResult] = useState<Awaited<
    ReturnType<typeof api.accounting.closeMonth>
  > | null>(null);

  const { data: periods } = useQuery({
    queryKey: ['acc-periods'],
    queryFn: () => api.accounting.periods(),
  });
    ...
    <div className="space-y-5">
      ...
      <Card>
        <CardContent className="flex flex-wrap items-end gap-3 p-4">
          ...
          <Button onClick={() => closeMut.mutate()} disabled={closeMut.isPending}>
            <CalendarCheck className="mr-1.5 h-4 w-4" /> Oyni yopish
          </Button>
        </CardContent>
      </Card>
        ...
        <Card>
          <CardContent className="space-y-3 p-4">
            <div className="font-semibold">
              {MONTHS[result.month - 1]} {result.year} — yopish natijasi
            </div>
            <div className="space-y-1.5 text-sm">
              <Check
                ok
                label={`Amortizatsiya hisoblandi: ${result.checklist.depreciation_posted} ta`}
              />
              <Check
                ok={result.checklist.gl_balanced}
                label={
                  result.checklist.gl_balanced
                    ? 'GL balansda (debit = kredit)'
                    : 'GL balans XATO — tekshiring!'
                }
              />
              <Check
                ok={result.checklist.payroll_posted}
                label="Maosh provodkalari GL'da (avtomatik)"
              />
            </div>
      ...
      <Card>
        <CardContent className="p-4">
          ...
          {(periods ?? []).length === 0 ? (
            <p className="text-muted-foreground text-sm">Hali yopilgan oy yo'q.</p>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="text-muted-foreground border-b text-left text-xs">
                  <th className="py-1">Davr</th>
                  <th>Holat</th>
              ...
              <tbody>
                {periods?.map((p) => (
                  <tr key={`${p.period_year}-${p.period_month}`} className="border-b last:border-0">
                    ...
                    <td>
                      {p.status === 'closed' ? (
                        <Badge variant="secondary" className="gap-1 bg-slate-500/15 text-[10px]">
                    ...
                    <td className="text-right font-medium tabular-nums">{fmt(p.net_profit_uzs)}</td>
                    <td className="text-right">
                      {p.status === 'closed' && (
                        <button
                          className="text-muted-foreground inline-flex items-center gap-1 text-xs hover:underline"
                          onClick={() =>
                            reopenMut.mutate({ year: p.period_year, month: p.period_month })
                          }
                        >

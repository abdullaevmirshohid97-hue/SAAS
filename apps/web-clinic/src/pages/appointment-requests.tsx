      ...
      <CardContent className="space-y-3 p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <div className="text-muted-foreground mt-1 text-sm">
              <a
                href={`tel:${req.patient_phone_snapshot}`}
                className="inline-flex items-center gap-1 hover:underline"
              >
                <Phone className="h-3 w-3" />
                {req.patient_phone_snapshot}
              </a>
              {req.doctor_name && (
                <>
                  {' '}
        ...
        <div className="bg-muted/40 space-y-1 rounded-md p-2 text-xs">
          {req.preferred_note && (
            <div>
              🕒 Qulay vaqt:{' '}
              <span className="text-foreground font-medium">{req.preferred_note}</span>
            </div>
          )}
          {req.preferred_at && (
            <div>
              📅 So'ralgan:{' '}
              <span className="text-foreground font-medium">{fmtTime(req.preferred_at)}</span>
            </div>
          )}
        ...
        {req.scheduled_at && (
          <div className="text-xs text-emerald-700 dark:text-emerald-400">
            ...
            Belgilangan: {fmtTime(req.scheduled_at)}
          </div>
        )}
        {req.response_note && (
          <div className="text-muted-foreground text-xs">Javob: {req.response_note}</div>
        )}
...
function ConfirmDialog({ req, onClose }: { req: Req; onClose: () => void }) {
  ...
  const mut = useMutation({
    mutationFn: () =>
      api.clinicAppointments.respond(req.id, {
        action: 'confirm',
  ...
  return (
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Navbatni tasdiqlash</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <div className="text-sm">
            {req.patient_name_snapshot} · {req.patient_phone_snapshot}
          </div>
          ...
          <div className="space-y-1">
            ...
            <Input
              ...
              placeholder="Masalan: 2-qavat, 5-xona"
            />
          </div>
        </div>
        <DialogFooter>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            ...
            <Input
              value={note}
              onChange={(e) => setNote(e.target.value)}
              placeholder="Masalan: bu vaqtda band"
            />
          </div>
        </div>
        <DialogFooter>

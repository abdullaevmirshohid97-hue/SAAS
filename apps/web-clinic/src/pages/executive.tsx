...
export function ExecutivePage() {
  ...
  const { data: ex } = useQuery({
    queryKey: ['exec', params],
    queryFn: () => api.accounting.executive(params),
  });
  const { data: top } = useQuery({
    queryKey: ['exec-top', params],
    queryFn: () => api.analytics.topServices(params),
  });
  const { data: cons } = useQuery({
    queryKey: ['exec-cons', params],
    queryFn: () => api.company.consolidated({ from: params.from, to: params.to }),
  });
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="CFO Dashboard"
        description="Bitta oynada: kassa, EBITDA, foyda, debitor/kreditor, inventar, top xizmat/xarajat, filial reytingi."
      />
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <Kpi
          ...
          label="Sof foyda"
          value={k?.profit ?? 0}
          tone={(k?.profit ?? 0) >= 0 ? 'good' : 'bad'}
        />
        <Kpi
          icon={<Banknote className="h-3.5 w-3.5" />}
          label="EBITDA"
          value={k?.ebitda ?? 0}
          tone={(k?.ebitda ?? 0) >= 0 ? 'good' : 'bad'}
        />
        <Kpi
          icon={<Users className="h-3.5 w-3.5" />}
          label="Bemor qarzi (AR)"
          value={k?.patient_ar ?? 0}
        />
        <Kpi
          icon={<ShieldCheck className="h-3.5 w-3.5" />}
          label="Sug'urta qarzi"
          value={k?.insurer_ar ?? 0}
        />
        <Kpi
          icon={<Truck className="h-3.5 w-3.5" />}
          label="Kreditor (AP)"
          value={k?.accounts_payable ?? 0}
          tone="bad"
        />
        <Kpi
          icon={<Boxes className="h-3.5 w-3.5" />}
          label="Inventar qiymati"
          value={k?.inventory_value ?? 0}
        />
        <Kpi
        ...
        <Card>
          <CardContent className="p-4">
            <div className="mb-2 flex items-center gap-2">
              ...
              <span className="font-semibold">Top xizmatlar</span>
            </div>
            {(top ?? []).length === 0 ? (
              <EmptyState title="Ma'lumot yo'q" description="Davrda sotuv yo'q." />
            ) : (
                  ...
                  <div key={i} className="flex items-center justify-between">
                    ...
                    <span className="flex items-center gap-3">
                      ...
                      {typeof s.margin_pct === 'number' && (s.cost ?? 0) > 0 && (
                        <span
                          className={`w-10 text-right text-xs tabular-nums ${s.margin_pct >= 50 ? 'text-emerald-600' : s.margin_pct >= 20 ? 'text-amber-600' : 'text-rose-600'}`}
                        >
                          {s.margin_pct}%
                        </span>
                      )}
        ...
        <Card>
          <CardContent className="p-4">
            <div className="mb-2 flex items-center gap-2">
              ...
              <span className="font-semibold">Top xarajatlar</span>
            </div>
            {(ex?.top_expenses ?? []).length === 0 ? (
              <EmptyState title="Ma'lumot yo'q" description="Davrda xarajat yo'q." />
            ) : (
              <div className="space-y-1 text-sm">
      ...
      {multiBranch && (
        <Card>
          <CardContent className="p-4">
            <div className="mb-2 flex items-center gap-2">
              <Trophy className="h-4 w-4 text-amber-600" />
              <span className="font-semibold">Filial reytingi (foyda)</span>
            </div>
            <div className="space-y-1 text-sm">
              {cons?.branches.map((b, i) => (
                <div key={b.clinic_id} className="flex items-center justify-between">
                  <span>
                    {i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `${i + 1}.`} {b.clinic_name}
                  </span>
                  <span
                    className={`font-semibold tabular-nums ${b.profit >= 0 ? '' : 'text-rose-600'}`}
                  >
                    {fmt(b.profit)}
                  </span>
                </div>

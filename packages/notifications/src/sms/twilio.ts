...
export class TwilioAdapter implements SmsAdapter {
  ...
  async send(input: SmsInput): Promise<SendResult> {
    const auth = Buffer.from(`${this.creds['account_sid']}:${this.creds['auth_token']}`).toString(
      'base64',
    );
    const res = await fetch(
      `https://api.twilio.com/2010-04-01/Accounts/${this.creds['account_sid']}/Messages.json`,
      {
        method: 'POST',
        headers: {
        ...
        body: new URLSearchParams({
          To: input.to,
          From: input.from ?? this.creds['from']!,
          Body: input.text,
        }),
      },
    );
